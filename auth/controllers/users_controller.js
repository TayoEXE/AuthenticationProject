        ...
        .exec(function(err, user) {
            if (!user) {
                ...
                req.session.regenerate(function() {
                    ...
                    req.session.username = user.username;
                    req.session.msg = 'Authenticated as ' + user.username;
                    req.session.favorite_book = user.favorite_book;
                    req.session.favorite_genre = user.favorite_genre;
                    req.session.book_cover = user.book_cover;
                    res.redirect('/');
                });
            }
            else {
...
exports.updateUser = function(req, res) {
    User.findOne({ _id: req.session.user })
        .exec(function(err, user) {
            user.set('email', req.body.email);
            user.set('favorite_book', req.body.favorite_book);
            user.set('favorite_genre', req.body.favorite_genre);
            user.set('book_cover', req.body.book_cover);
            user.save(function(err) {
                if (err) {
